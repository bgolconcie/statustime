...
export const api = {
  ...
  register: (name: string, email: string, password: string) =>
    req<{ token: string }>('/api/auth/register', { method: 'POST', body: JSON.stringify({ name, email, password }) }),
  ...
  updateLeave: (id: string, status: string) =>
    req('/api/dashboard/leave/' + id, { method: 'PATCH', body: JSON.stringify({ status }) }),
