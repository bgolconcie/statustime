...
// Hourly heatmap data: returns all sessions with start_time/end_time for the date range
router.get('/users/:userId/sessions-hourly', auth, async (req, res) => {
...
router.get('/users/:userId/sessions', auth, async (req, res) => {
  ...
  try {
    const result = await db.query(
      `SELECT date, start_time, end_time, duration_minutes, status FROM time_sessions
...
router.get('/users/:userId/stats', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ totalMinutes: totalMins, activeDays, avgPerActiveDay, mostActiveDay: maxRow ? { date: maxRow.date, minutes: parseInt(maxRow.total_minutes) } : null, daysRange: parseInt(days) });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
...
router.get('/presence', auth, async (req, res) => {
  try {
    const integrations = await db.query(
      `SELECT i.access_token, i.platform, tu.id as user_id, tu.platform_user_id FROM integrations i JOIN tracked_users tu ON tu.integration_id=i.id WHERE i.org_id=$1 AND tu.is_active=true`,
      [req.org.id]
    );
    const presenceMap = {};
    const byToken = {};
...
router.get('/users/:userId/presence', auth, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT i.access_token, i.platform, tu.platform_user_id FROM tracked_users tu JOIN integrations i ON tu.integration_id=i.id WHERE tu.id=$1 AND tu.org_id=$2`,
      [req.params.userId, req.org.id]
    );
...
router.get('/hours', auth, async (req, res) => {
  ...
  try {
    let q = `SELECT tu.id,tu.display_name,tu.avatar_url,ts.date,SUM(ts.duration_minutes) as total_minutes FROM time_sessions ts JOIN tracked_users tu ON ts.user_id=tu.id WHERE ts.org_id=$1 AND ts.date>=CURRENT_DATE-$2 AND ts.duration_minutes IS NOT NULL`;
...
router.get('/export', auth, async (req, res) => {
  ...
  try {
    const result = await db.query(
      `SELECT tu.display_name,tu.email,i.platform,i.team_name,ts.date,SUM(ts.duration_minutes) as total_minutes,ROUND(SUM(ts.duration_minutes)::numeric/60,2) as total_hours
    ...
    const csv = ['Name,Email,Platform,Team,Date,Minutes,Hours', ...result.rows.map(r => `"${r.display_name}","${r.email||''}","${r.platform}","${r.team_name}","${r.date}",${r.total_minutes},${r.total_hours}`)].join('\n');
...
router.get('/leave', auth, async (req, res) => {
  try {
    const result = await db.query(`SELECT lr.*,tu.display_name,tu.avatar_url FROM leave_requests lr JOIN tracked_users tu ON lr.user_id=tu.id WHERE lr.org_id=$1 ORDER BY lr.created_at DESC LIMIT 50`,[req.org.id]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
